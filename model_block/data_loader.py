...
################################################################################################################
class IMDB_and_REDDIT_DataSet(Dataset):
    def __init__(self, file_dir, shuffle=False, seed=27):

        with open(os.path.join(file_dir, "adjacency_matrix_int8.json"), 'r') as f:
            self.original_adjs = json.load(f)
        print("Original graphs loaded!")

        vertex_features = np.load(os.path.join(file_dir, "vertex_feature.npy")).astype(np.float32)
        vertex_features = preprocessing.scale(vertex_features)
        self.vertex_features = torch.FloatTensor(vertex_features)
        ...
        with open(os.path.join(file_dir, "vertex_id.json"), 'r') as f:
            self.vertices = json.load(f)
        print("Vertex IDs loaded!")

        if shuffle:
            self.original_adjs, self.labels, self.vertices, = \
                sklearn.utils.shuffle(self.original_adjs,
                                      self.labels,
                                      self.vertices,
                                      random_state=seed)
        
        self.N = len(self.original_adjs)
        n_classes = self.get_num_class()
    ...
    def get_feature_dimension(self):
        return self.vertex_features.size(1)
    ...
    def __getitem__(self, idx):
        if isinstance(idx, list):
            o_adj = [torch.from_numpy(np.array(self.original_adjs[i]).astype(np.float32)) for i in idx]
            vertices = [torch.from_numpy(np.array(self.vertices[i])) for i in idx]
            return o_adj, torch.from_numpy(self.labels[idx]), torch.from_numpy(self.labels[idx]), vertices
        o_adj = np.array(self.original_adjs[idx]).astype(np.float32)
        vertices = np.array(self.vertices[idx])
        return o_adj, self.labels[idx], self.labels[idx], vertices


def load_IMDB_and_REDDIT_dataset(path, train_ratio, val_ratio, batch_size, shuffle, seed, num_workers=10):
    inf_ds = IMDB_and_REDDIT_DataSet(path, shuffle, seed)
    class_weight = inf_ds.get_class_weight()
    n_feat = inf_ds.get_feature_dimension()
